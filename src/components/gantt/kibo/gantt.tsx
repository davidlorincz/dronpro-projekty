          ...
          <GanttContentHeader
            ...
            renderHeaderItem={(item: number) => (
              <div className="flex items-center justify-center gap-1">
                <p>
                  {format(addDays(new Date(year.year, index, 1), item), "d", { locale: cs })}
                </p>
            ...
            title={format(new Date(year.year, index, 1), "LLLL yyyy", { locale: cs })}
          />
          <GanttColumns
            columns={month.days}
  ...
  return gantt.timelineData.map((year) => (
    <div className="relative flex flex-col" key={year.year}>
      <GanttContentHeader
        ...
        renderHeaderItem={(item: number) => (
          <p>{format(new Date(year.year, item, 1), "LLL", { locale: cs })}</p>
        )}
        title={`${year.year}`}
      />
      <GanttColumns
    ...
    year.quarters.map((quarter, quarterIndex) => (
      ...
      >
        <GanttContentHeader
          ...
          renderHeaderItem={(item: number) => (
            <p>
              {format(new Date(year.year, quarterIndex * 3 + item, 1), "LLL", { locale: cs })}
            </p>
          )}
...
export const GanttSidebarHeader: FC = () => (
  ...
  >
    ...
    <p className="flex-1 truncate text-left">Projekt / subúkol</p>
    <p className="shrink-0">Termín</p>
  </div>
);
    ...
    >
      ...
      >
        ...
        <div className="whitespace-nowrap rounded-full border border-border/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg">
          {formatDate(date, "d. M. yyyy", { locale: cs })}
        </div>
      </div>
    </div>
  );
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <div
      className="h-full w-full rounded-md border bg-background p-2 text-xs shadow-sm"
      style={color ? { borderColor: color, background: `color-mix(in srgb, ${color} 22%, var(--a-surface))` } : undefined}
    >
      <div
        className={cn(
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  // Sync s props (reaktivní data z Convexu)
...
> = memo(({ label, date, id, onRemove, className }) => {
  ...
  return (
    ...
    >
      <div
        onDoubleClick={onRemove ? handleRemove : undefined}
        className={cn(
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  // Počáteční scroll: dnešek zhruba v první třetině viditelné části.
  useEffect(() => {
    if (scrollRef.current) {
      const now = new Date();
      const start = new Date(timelineData.at(0)?.year ?? now.getFullYear(), 0, 1);
      const parsedColumnWidth = (columnWidth * zoom) / 100;
      const todayOffset =
        getDifferenceIn(range)(now, start) * parsedColumnWidth +
        calculateInnerOffset(now, range, parsedColumnWidth);
      const sidebar = 300;
      scrollRef.current.scrollLeft = Math.max(
        0,
        sidebar + todayOffset - scrollRef.current.clientWidth / 3
      );
      setScrollX(scrollRef.current.scrollLeft);
    }
  }, [setScrollX, timelineData, columnWidth, zoom, range]);

  // Update sidebar width when DOM is ready
  useEffect(() => {
    const updateSidebarWidth = () => {
      const sidebarElement = scrollRef.current?.querySelector<HTMLElement>(
        '[data-roadmap-ui="gantt-sidebar"]'
      );
      // Skutečná šířka sidebaru (Kibo měl natvrdo 300)
      const newWidth = sidebarElement ? sidebarElement.getBoundingClientRect().width || 300 : 0;
